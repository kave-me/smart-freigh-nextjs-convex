...
export default function EnrichmentPanel({
  ...
  const handleItemSelect = (item: any) => {
    setEditedItem({
      ...
      totalCost: item.unitCost * editedItem.quantity
    });
  ...
  const enrichmentOptions = [
    { id: "1", className: "bg-green-500", description: "3-in-1 Airline Set with Gladhands (15 feet)", unitCost: 165 },
    { id: "2", className: "bg-orange-500", description: "2-in-1 Airline Set with Gladhands (15 feet)", unitCost: 100 },
    { id: "3", className: "bg-red-500", description: "None", unitCost: 0 }
  ];

  return (
    <Card className="fixed inset-0 z-50 bg-white dark:bg-gray-950 overflow-auto" style={{ maxWidth: "800px", maxHeight: "80vh", margin: "auto", top: "10vh" }}>
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="grid gap-4">
          ...
          <RadioGroup value={selectedOption} className="space-y-3">
            {enrichmentOptions.map((option) => (
              <div key={option.id} className="flex items-center space-x-3 p-3 border rounded-md">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${option.className}`}>
                  {option.id}
                </div>
                <RadioGroupItem 
        ...
        <div>
          ...
          <Input
            ...
            readOnly
          />
          <p className="text-sm text-gray-500 mt-1">Automatically calculated based on quantity and unit cost</p>
        </div>
